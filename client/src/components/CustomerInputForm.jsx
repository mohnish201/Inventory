...
import { addCustomer, updateCustomer } from "../actions/customer.action";
...
const CustomerInputForm = ({ defaultValue, clear }) => {
  ...
  const onUpdate = async () => {
    ...
    try {
      await updateCustomer(payload, defaultValue._id);
      toast.success("Success");
    } catch (error) {
      toast.error(error.message);
    }
  ...
  useEffect(() => {
    if (defaultValue) {
      setCustomerType(defaultValue.customerType);
    ...
    <Stack pb={32}>
      <Stack spacing={4} p={4}>
        <Text fontSize={24} fontWeight={500}>
          {" "}
          {defaultValue ? "Update Customer" : "New Customer"}{" "}
        </Text>
        ...
        <SimpleGrid columns={[1, 1, 3, 4]} gap={2}>
          <Select
            ...
            value={customerType}
            onChange={({ target }) => setCustomerType(target.value)}
          >
